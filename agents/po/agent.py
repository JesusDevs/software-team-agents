...
from agents.po.prompt import get_system_prompt
...
def po_agent_node(state: ProjectState, config: RunnableConfig) -> dict:
    run_id = state.get("run_id", "default")
    llm = get_model_for_agent(ROLE).bind_tools(TOOLS)
    system_prompt = get_system_prompt(state)
    task = state.get("task_instructions") or state.get("project_brief", "Start the PRD")
    ...
    handoff_log.append({
        ...
        "from_agent": ROLE,
        "to_agent": "hitl_gate",
        "message": f"Completed work on {ROLE.upper()} phase",
