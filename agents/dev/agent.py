...
from agents.dev.prompt import get_system_prompt
...
def dev_agent_node(state: ProjectState, config: RunnableConfig) -> dict:
    run_id = state.get("run_id", "default")
    llm = get_model_for_agent(ROLE).bind_tools(TOOLS)
    task = state.get("task_instructions") or "Create the implementation specification."
    ...
    ensure_artifact_saved(ROLE, "04_IMPLEMENTATION_SPEC.md", run_id, messages)
