...
st.set_page_config(page_title="Artefactos", page_icon="📄", layout="wide")
st.title("📄 Artefactos")
st.caption("Entregables generados por cada agente.")
...
if not artifacts_dir.exists() or not list(artifacts_dir.glob("*.md")):
    st.info("Aún no hay artefactos. Inicia el pipeline desde la página **Pipeline**.")
    st.stop()
...
for tab, md_file in zip(tabs, md_files):
    with tab:
        ...
        with col1:
            st.caption(f"📁 `{md_file}` — {len(content):,} caracteres")
        with col2:
            st.download_button(
                label="⬇️ Descargar",
                data=content,
