...
from agents.architect.prompt import get_system_prompt
...
def architect_agent_node(state: ProjectState, config: RunnableConfig) -> dict:
    run_id = state.get("run_id", "default")
    llm = get_model_for_agent(ROLE).bind_tools(TOOLS)
    task = state.get("task_instructions") or "Create the system design document."
    ...
    ensure_artifact_saved(ROLE, "03_SYSTEM_DESIGN.md", run_id, messages)
