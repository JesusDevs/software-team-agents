...
from agents.ux.prompt import get_system_prompt
...
def ux_agent_node(state: ProjectState, config: RunnableConfig) -> dict:
    run_id = state.get("run_id", "default")
    llm = get_model_for_agent(ROLE).bind_tools(TOOLS)
    task = state.get("task_instructions") or "Create the UX specification based on the PRD."
    ...
    ensure_artifact_saved(ROLE, "02_UX_SPEC.md", run_id, messages)
